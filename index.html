...
import { Sword, Shield, Heart, Zap, TrendingUp, ShoppingCart, Users, DollarSign, Gift, Star, Crown, Sparkles, Flame } from 'lucide-react';

const RPGGame = () => {
  // Load/Save game state
  const loadGameState = () => {
    try {
      const saved = window.storage ? null : localStorage.getItem('rpgGameState');
  ...
  const watchAd = (type) => {
    ...
    // Khi có backend, gọi API Adsterra để hiển thị quảng cáo
  ...
  const handleWithdraw = (method, amount, details) => {
    // PLACEHOLDER: Cần backend và giấy phép hợp pháp
    // Đây chỉ là giao diện demo
    if (amount < (method === 'card' ? 10000 : 20000)) {
      addLog('⚠️ Số tiền rút tối thiểu không đủ!', 'error');
      return;
    }

    if (player.gold < amount) {
      addLog('⚠️ Không đủ vàng để rút!', 'error');
      return;
    }
    ...
    setWithdrawHistory(prev => [withdrawal, ...prev]);
    const updatedPlayer = { ...player, gold: player.gold - amount };
    setPlayer(updatedPlayer);
    saveGame(updatedPlayer);
    
    addLog('✅ Yêu cầu rút tiền đã gửi! Chờ xử lý...', 'success');
      ...
      <div className="bg-black/30 border-b border-purple-500/30">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex gap-2 overflow-x-auto py-2">
            {[
              { id: 'game', label: '⚔️ Chiến Đấu', icon: Sword },
              { id: 'upgrade', label: '⬆️ Nâng Cấp', icon: TrendingUp },
              { id: 'shop', label: '🛒 Cửa Hàng', icon: ShoppingCart },
              { id: 'boss', label: '👑 Boss', icon: Crown },
              { id: 'ads', label: '📺 Quảng Cáo', icon: Gift },
              { id: 'market', label: '🏪 Chợ', icon: Users },
              { id: 'withdraw', label: '💵 Rút Tiền', icon: DollarSign },
            ].map(nav => (
              <button
                key={nav.id}
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-red-900/30 rounded-xl p-4 border-2 border-red-500/50">
                    <div className="text-center">
                      ...
                      <h3 className="font-bold text-xl text-red-300">{monster.name}</h3>
                      <div className="mt-2">
                  ...
                  >
                    {playerTurn ? '⚔️ TẤN CÔNG!' : '⏳ Đợi lượt...'}
                  </button>
                </div>
              )}
            </div>
        ...
        {currentView === 'withdraw' && (
          <div className="max-w-4xl mx-auto">
            ...
            <div className="text-center">
              <button
                onClick={() => setCurrentView('withdraw-history')}
        ...
        {currentView === 'withdraw-history' && (
          <div className="max-w-4xl mx-auto">
            ...
            <div className="bg-gradient-to-br from-blue-900/50 to-purple-900/50 rounded-xl p-6 border-2 border-blue-500/30">
              {withdrawHistory.length === 0 ? (
                <p className="text-center text-purple-300 py-8">Chưa có lịch sử rút tiền</p>
              ) : (
                <div className="space-y-3">
                  {withdrawHistory.map(item => (
                    <div key={item.id} className="bg-black/30 p-4 rounded-lg">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-bold">{item.method === 'bank' ? '🏦 Ngân Hàng' : item.method === 'momo' ? '📱 MoMo' : '🎫 Thẻ Cào'}</p>
                          <p className="text-sm text-purple-300">{item.date}</p>
                          <p className="text-sm text-gray-400">Số tiền: {item.amount.toLocaleString()}đ</p>
                        </div>
